...
def main():
    ...
    with calc_right:
        st.markdown(
            ...
            f'    <div style="font-size:0.95rem; font-weight:bold; color:#22c55e;">Offence{f" — £{offence_amount:,.2f}" if monthly_amount > 0 else ""} ({offence_override}%)</div>'
            ...
            f'    <div style="font-size:0.95rem; font-weight:bold; color:#3b82f6;">Defence{f" — £{defence_amount:,.2f}" if monthly_amount > 0 else ""} ({defence_override}%)</div>'
            f'    <div style="margin-top:0.6rem;">'
